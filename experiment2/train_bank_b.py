...
def run_bank_b():
    print("\n--- 🏦 BANK B (Equity Simulator) STARTED ---")
    
    # 1. Load Data
    raw_df = pd.read_csv('experiment2/bank_b_raw.csv')
    
    # 2. Adapter (FIXING THE MESSED UP COLUMNS)
    # Bank B calls 'amount' -> 'txn_val' and 'oldbalanceOrg' -> 'old_bal'
    config_map = {'txn_val': 'amount', 'old_bal': 'oldbalanceOrg'}
    
    adapter = DataHarmonizer(schema_map=config_map)
    ...
    print(f"[Bank B] Training on {len(X)} rows...")
    model.train(epoch=3, train_dataset=ms_dataset)
    
    # 5. Evaluate & Log
    acc = model.eval(ms_dataset)
    print(f"[Bank B] Local Accuracy: {acc}")
    
    with open("experiment2/results.txt", "a") as f:
        f.write(f"Bank B Local Accuracy: {acc['acc']}\n")
    
    save_checkpoint(net, "experiment2/bank_b.ckpt")
    print("✅ Bank B Complete.")
