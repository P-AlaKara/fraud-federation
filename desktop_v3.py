...
class MainApp(ctk.CTk):
    ...
    def setup_train_tab(self):
        # Use ScrollableFrame so you can scroll down to the progress bar
        frame = ctk.CTkScrollableFrame(self.tab_train, fg_color="transparent")
        frame.pack(fill="both", expand=True, padx=15, pady=15)
        ...
        db_card = ctk.CTkFrame(frame, fg_color=COLORS['bg_light'], corner_radius=10)
        ...
        ctk.CTkLabel(
            db_card,
            text="Connect directly to your database server (Read-Only)",
            font=("Segoe UI", 10),
            text_color=COLORS['text_gray']
        ).pack(pady=(0, 10))
        ...
        self.db_selector.set("Select Database Source...")
        self.db_selector.pack(pady=(0, 10), padx=20)
        
        self.btn_db_connect = ctk.CTkButton(
            db_card,
        ...
        self.btn_db_connect.pack(pady=(0, 15), padx=20)
    ...
    def db_connect(self):
        ...
        if selected_db == "Select Database Source...":
            messagebox.showwarning("Connection Error", "Please select a valid database server.")
            return
            
        # Simulate a connection attempt for the demo
        self.log(f"Attempting secure handshake with {selected_db}...")
        
        # Show success but explain it's a demo
        messagebox.showinfo(
            "Connection Established", 
            f"Secure tunnel established with {selected_db}.\n\n"
            "NOTE: For this hackathon demo, please use the CSV option above "
            "to load the sample transaction data."
        )
        self.log(f"Connected to {selected_db} (Read-Only Mode)")
    ...
    def setup_sync_tab(self):
        ...
        # Upload Card
        upload_card = ctk.CTkFrame(frame, fg_color=COLORS['bg_light'], corner_radius=10)
        ...
        ctk.CTkLabel(
            upload_card,
            text="Upload your encrypted model weights",
            font=("Segoe UI", 10),
            text_color=COLORS['text_gray'],
            wraplength=450
        ).pack(pady=(0, 15), padx=20)
        ...
        ctk.CTkButton(
            upload_card,
            text="⬆ Upload Latest Weights",
            height=40,
            font=("Segoe UI", 12),
            fg_color=COLORS['success'],  # 
            hover_color=COLORS['success_hover'],
            command=self.upload_weights
        ).pack(fill="x", padx=30, pady=(0, 20))
